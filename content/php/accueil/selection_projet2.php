...
if($userinfo['type_compte']=='Administrateur Logiciel') {
    $search_projet = $bdd->prepare("SELECT F_projet.id_projet, F_projet.nom_projet, F_projet.id_projet_gen FROM F_projet INNER JOIN ZD_projet_gen ON F_projet.id_projet = ZD_projet_gen.id_projet_desc_current");
    $search_projet->execute();
}
// Si pas utilisateur admin alors chercher la liste des projet où l'utilisateur est chef de projet
else {
    $search_projet = $bdd->prepare("SELECT F_projet.id_projet, F_projet.nom_projet, F_projet.id_projet_gen FROM F_projet INNER JOIN ZD_projet_gen ON F_projet.id_projet = ZD_projet_gen.id_projet_desc_current WHERE F_projet.id_utilisateur=?");
