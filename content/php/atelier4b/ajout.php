...
$nomscenar = $_POST['nomscenar'];
$modeope = $_POST['modeope'];
$id_mode_operatoire = "id_mode_operatoire";



$recupere = $bdd->prepare("SELECT id_scenario_operationnel FROM scenario_operationnel WHERE description_scenario_operationnel = ?");
...
if ($results["error"] === false && isset($_POST['validerope'])) {
  $recupere->bindParam(1, $nomscenar);
  $recupere->execute();
  $id_scenar = $recupere->fetch();
  $insere->bindParam(1, $id_mode_operatoire);
  $insere->bindParam(2, $modeope);
  $insere->bindParam(3, $id_scenar[0]);
  $insere->execute();
?>
